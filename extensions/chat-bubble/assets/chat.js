...
(() => {
  ...
  function createOrderStatus(block2, { onReorder } = {}) {
    const o = normalize(block2);
    const node = el("div", { class: "swa-order" });
    const header = el("div", { class: "swa-order-header" });
    header.appendChild(
      el(
        "div",
        { class: "swa-order-number" },
        `Order ${o.orderLabel}${o.dateLabel ? " \xB7 " + o.dateLabel : ""}`
      )
    );
    ...
    if (o.cancelled) {
      node.appendChild(
        el(
          "div",
          { class: "swa-order-cancel-banner" },
          o.cancelReason ? `Cancelled \u2014 reason: ${o.cancelReason.toLowerCase()}.` : "This order was cancelled."
        )
      );
    }
    ...
    if (o.fulfillments.length > 0) {
      ...
      o.fulfillments.forEach((f, i) => {
        ...
        head.appendChild(
          el(
            "span",
            { class: "swa-order-parcel-label" },
            o.fulfillments.length > 1 ? `Parcel ${i + 1} of ${o.fulfillments.length}` : "Shipment"
          )
        );
        if (f.trackingCompany) {
          head.appendChild(
            el("span", { class: "swa-order-parcel-carrier" }, "\xB7 " + f.trackingCompany)
          );
        }
        parcel.appendChild(head);
        if (f.items && f.items.length > 0) {
          const lines = f.items.map((it) => `${it.title} \xD7 ${formatQty(it.quantity)}`).join(", ");
  ...
  function normalize(block2) {
    const order = block2 && block2.order ? block2.order : null;
    const iso = order?.processedAt || block2?.processedAt;
    const dateLabel = iso ? formatShortDate(iso) : block2?.date || "";
    const fulfillmentRaw = order?.fulfillmentStatus || block2?.status || "unfulfilled";
    ...
    const items = order?.lineItems && order.lineItems.length > 0 ? order.lineItems : block2?.items || [];
    const fulfillments = order?.fulfillments || [];
    const cancelled = !!(order?.cancelledAt || order?.cancelReason);
    const cancelReason = order?.cancelReason || null;
    const orderLabel = order?.name ? order.name : block2?.orderNumber ? `#${block2.orderNumber}` : "";
    const totalAmount = typeof order?.totalAmount === "number" ? order.totalAmount : typeof block2?.total === "number" ? block2.total : null;
    const currency = order?.currency || block2?.currency || "USD";
    return {
      orderLabel,
      dateLabel,
      statusLabel,
      ...
      legacyTrackingUrl: block2?.trackingUrl || null
    };
  }
  ...
  function financialStatusBadge(raw) {
    ...
    switch (s) {
      case "PAID":
        return null;
      // expected default — don't double-up with a badge
      case "REFUNDED":
        return { label: "Refunded", tone: "critical" };
      case "PARTIALLY_REFUNDED":
        return { label: "Partial refund", tone: "warning" };
      case "PENDING":
      ...
      case "VOIDED":
        return { label: "Voided", tone: "critical" };
      default:
        return null;
    }
  }
